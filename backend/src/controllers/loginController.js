...
export const login = async (req, res) => {
  ...
  try {
    ...
    res.cookie("token", token, {
      httpOnly: true,
      secure: process.env.NODE_ENV === "production", // HTTPS required on Render
      sameSite: "Strict", // same-domain cookie
...
export const createAccount = async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ message: "User and Customer created successfully", token });
...
export const createServiceProvider = async (req, res) => {
  try {
    ...
    const hashedPassword = await bcrypt.hash(password, 10);
    const newUser = new User({ username, email, password: hashedPassword, role: 'provider' });
